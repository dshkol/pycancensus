...
# Test configuration
API_KEY = (
    os.environ.get("CANCENSUS_API_KEY")
    or "CensusMapper_7cb8d0ee55b67305388e0a7e8ba9c725"
)
...
class TestComprehensiveScenarios:
    ...
    def test_scenario_1_provincial_population_analysis(self):
        ...
        assert "v_CA21_1" in data.columns or any(
            "Population" in col for col in data.columns
        ), "Population data missing"
        assert (
            data["Population"].sum() > 10000000
        ), "Total population seems too low for selected provinces"
        ...
        print(
            f"   📈 Most populous: {max_pop['Region Name']} ({max_pop['Population']:,})"
        )
        print(
            f"   📉 Least populous: {min_pop['Region Name']} ({min_pop['Population']:,})"
        )
    ...
    def test_scenario_2_toronto_demographic_breakdown(self):
        ...
        assert (
            len(data) > 20
        ), f"Expected many municipalities in Toronto CMA, got {len(data)}"
        assert any(
            "Population" in col for col in data.columns
        ), "Population data missing"
    ...
    def test_scenario_3_income_inequality_analysis(self):
        ...
        income_vectors = pc.search_census_vectors(
            "median household income", "CA21", quiet=True
        )
        ...
        if len(income_vectors) == 0:
            ...
            income_vectors = income_vectors[
                income_vectors["label"].str.contains("median", case=False, na=False)
            ]
        ...
        major_cmas = {
            "CMA": [
                ...
                "59933",
                "505",
            ]  # Toronto, Montreal, Calgary, Vancouver, Ottawa
        }
    ...
    def test_scenario_4_vector_hierarchy_navigation(self):
        ...
        ), "parent_census_vectors should return DataFrame"
        assert isinstance(
            children, pd.DataFrame
        ), "child_census_vectors should return DataFrame"
        assert isinstance(
            housing_vectors, pd.DataFrame
        ), "find_census_vectors should return DataFrame"
    ...
    def test_scenario_5_multi_dataset_comparison(self):
        ...
        try:
            ...
            if len(data_2021) > 0 and len(data_2016) > 0:
                pop_2021 = (
                    data_2021["Population"].iloc[0]
                    if "Population" in data_2021.columns
                    else data_2021.iloc[0, -1]
                )
                pop_2016 = (
                    data_2016["Population"].iloc[0]
                    if "Population" in data_2016.columns
                    else data_2016.iloc[0, -1]
                )
                ...
                assert (
                    abs(growth) < 50
                ), f"Population growth seems unrealistic: {growth}%"
    ...
    def test_scenario_6_geography_with_data(self):
        ...
        try:
            ...
            assert (
                len(geo_data) > 5
            ), f"Expected multiple municipalities, got {len(geo_data)}"
            assert any(
                "Population" in col for col in geo_data.columns
            ), "Population data missing"
