...
Or get a free key at: https://censusmapper.ca/users/sign_up
    ...
    parser.add_argument(
        "--cache-path",
        help="Path for caching downloaded data"
    )
    parser.add_argument(
        "--no-cache",
        action="store_true",
    ...
    search_regions_parser = subparsers.add_parser("search-regions", help="Search for regions")
    search_regions_parser.add_argument("term", help="Search term")
    search_regions_parser.add_argument("dataset", help="Dataset identifier (e.g., CA16)")
    search_regions_parser.add_argument("--level", help="Filter by aggregation level")
    
    # Search vectors command
    search_vectors_parser = subparsers.add_parser("search-vectors", help="Search for vectors")
    search_vectors_parser.add_argument("term", help="Search term")
    search_vectors_parser.add_argument("dataset", help="Dataset identifier (e.g., CA16)")
    ...
    get_data_parser.add_argument("--regions", required=True, help="Regions as JSON (e.g., '{\"CMA\": \"59933\"}')")
