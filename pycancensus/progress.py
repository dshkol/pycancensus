...
class ProgressIndicator:
    ...
    def update(self, status: Optional[str] = None):
        ...
        if status:
            print(f"\r{spinner} {self.description} - {status} ({elapsed:.1f}s)", end="", flush=True)
        else:
            print(f"\r{spinner} {self.description}... ({elapsed:.1f}s)", end="", flush=True)
...
class DataSizeEstimator:
    ...
    @staticmethod
    def estimate_request_size(num_regions: int, num_vectors: int, level: str, geo_format: Optional[str] = None) -> dict:
        """
...
def show_request_preview(regions: dict, vectors: list, level: str, dataset: str, geo_format: Optional[str] = None, quiet: bool = False):
    """
    ...
    print(f"🔍 Estimated Size: {estimate['size_category']} ({estimate['estimated_rows']:,} rows)")
...
def create_progress_for_request(regions: dict, vectors: list, level: str, geo_format: Optional[str] = None) -> Optional[ProgressIndicator]:
    """
    ...
    if estimate['size_category'] in ['medium', 'large', 'very_large']:
        if geo_format == "geopandas":
            description = f"Downloading {estimate['estimated_rows']:,} regions with geography"
        else:
...
def show_download_progress(description: str = "Downloading data", show_spinner: bool = True) -> ProgressIndicator:
