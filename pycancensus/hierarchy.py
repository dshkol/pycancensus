...
def find_census_vectors(
    ...
    if search_type == "exact":
        ...
        mask = (
            (all_vectors['vector'].str.lower() == query_lower) |
        ...
        try:
            ...
            mask = (
                all_vectors['label'].str.contains(pattern, na=False) |
                all_vectors['vector'].str.contains(pattern, na=False)
            )
...
def _infer_parent_vector(vector: str, all_vectors: pd.DataFrame) -> Optional[Dict]:
    ...
    for potential_parent_num in range(1, vector_num):
        ...
        if not parent_match.empty:
            ...
            current_label = all_vectors[all_vectors['vector'] == vector]['label'].iloc[0].lower()
...
def _infer_child_vectors(vector: str, all_vectors: pd.DataFrame) -> List[Dict]:
    ...
    max_search = min(vector_num + 1000, all_vectors['vector'].str.extract(r'v_[A-Z0-9]+_(\d+)')[0].astype(int).max())
